...
\infer[()]{e[T] → e'[T]}{e → e'} \vspace{1em}
\infer[()]{(λα.e)[T] → e[α := T]}{} \vspace{1em}
\subsubsection{Types}
\infer[()]{Γ \vdash e[T] : T₁[α := T] }{Γ \vdash e: ∀α. T₁} \vspace{1em}
\infer[()]{Γ \vdash λα.e : ∀α.T}{Γ,α \vdash e: T} \vspace{1em}
...
\infer[(Application)]{Γ \vdash e₁ e₂ : β | \{T₁ = α → β, T₂ = α\} ∪ C₁ ∪ C₂}{Γ \vdash e₁ : α → β | C₁ \hspace{1em} Γ \vdash e₂ : T₂ | C₂} \vspace{1em}
